...
#include "gdbcmd.h"		/* for add_set_cmd and friends */
#include "symfile.h"
#include "objfiles.h"
#include "buildsym.h"
#include "stabsread.h"
...
/* Unless this is non-zero, in which case N_LBRAC and N_RBRAC entries
   are relative to the file start, not the function start */

static int block_file_relative = 0;

     ...
     struct objfile *objfile;
{
  ...
  switch (type)
	...
	break;
      ...
      if (block_file_relative)
	valu += last_source_start_addr; /* Relocate for dynamic loading (?). */
      else
	{
	  if (block_address_function_relative)
...
_initialize_dbxread ()
{
  ...
  add_show_from_set (
    add_set_cmd ("block-file-relative", class_maintenance, var_zinteger,
		 (char *)&block_file_relative,
		 "Set whether block debug info is file relative\n\
When non-zero, N_LBRAC and N_RBRAC as treated as file relative.",
		 &setlist),
      &showlist);
}

...
#  endif /* NEED_UNDERSCORE */

/*
 * Assembly function start definition
 */
#  define FUNC_START(name)\
	.global SYM_NAME(name);\
	.align  4;\
SYM_NAME(name):
...
#  define ARM_PSR_THUMB_STATE    0x00000020  /* Thumb/Arm Execution                    */
#  define ARM_PSR_MODE_MASK      0x0000001F  /* Arm Processor Mode Mask                */
#else /* ! defined(__ASSEMBLER__) */
  struct psr_struct {
      unsigned mode      : 5;
      unsigned t_bit     : 1;
...
#  define __sti()      asm volatile("                         \
         ...
         msr    cpsr, r0                                    \n\
         ldmfd  sp!, {r0}                                   \n")
#  define __mcr(cp_num, opcode1, ret_code, CRn, CRm, opcode2) \
     asm volatile (" mcr " cp_num  ", " \
                           opcode1 ", " \
                           "%0"    ", " \
                           CRn     ", " \
                           CRm     ", " \
                           opcode2 : /* no outputs */ : "r" (ret_code))
#  define __mrc(cp_num, opcode1, ret_code, CRn, CRm, opcode2) \
     asm volatile (" mrc " cp_num  ", " \
                           opcode1 ", " \
                           "%0"    ", " \
                           CRn     ", " \
                           CRm     ", " \
                           opcode2 : "=r" (ret_code) : /* no inputs */)
...
/*
 * Arm Cache and MMU Control Registers
 *
...
#  define ARM_CLEAN_CACHE_DATA_ENTRY_RM          cA
#  define ARM_DRAIN_CACHE_WRITE_BUFFER_OPCODE    0x4
#  define ARM_DRAIN_CACHE_WRITE_BUFFER_RM        cA
#else /* __ASSEMBLER__ */                        
#  define ARM_FLUSH_CACHE_INST_DATA_OPCODE       "0x0"
#  define ARM_FLUSH_CACHE_INST_DATA_RM           "c7"
#  define ARM_FLUSH_CACHE_INST_OPCODE            "0x0"
...
/*
 * Arm First Level Descriptor Format Definitions
 */
...
#endif /* __ASSEMBLER__ */
#define ARM_FIRST_LEVEL_PAGE_SIZE               0x4000   /* 16K */

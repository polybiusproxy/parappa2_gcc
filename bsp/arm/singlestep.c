/*
 * singlestep.c -- ARM specific single-step support.
 *
...
insert_ss_break(unsigned long *pc)
{
    ...
    {
        /*
         * All arm instructions are on a word boundary.
                                             ...
                                             unsigned shift, unsigned Rm)
{
    ...
    /*
     * According to the ARM Manual, if Rm is PC then,
                                                  ...
                                                  unsigned offset)
{
    ...
    /*
     * According to the ARM Manual, if Rn is PC then,
     * the value used is the address of the current instruction
     * plus 8
     */
    if (Rn == REG_PC)
...
{
    /*
     * According to the ARM Manual, if Rd is PC then
     * the result is unpredictable.  Don't do anything
     * here.  Just return.
     */
}
...
static void decode_lsio_inst(ex_regs_t *regs, union arm_insn inst)
{
     ...
     *
     * According to the Arm manual, automatic updates of the PC

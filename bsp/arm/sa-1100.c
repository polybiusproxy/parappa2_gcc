/*
 * sa-1100.c -- Intel StrongArm SA-1100 specific support for Cygnus BSP
 *
...
static void uart_putchar(void *base, char c)
{
    ...
    while ((*UART_STATUS_1(base) & SA1100_UART_TX_BUSY) == SA1100_UART_TX_BUSY)
        ;
...
void __dcache_flush(void *__p, int __nbytes)
{
    register volatile unsigned char *ptr =
        (volatile unsigned char *)(SA1100_ZEROS_BANK_BASE);
    register volatile unsigned char *ptr_stop =
        (volatile unsigned char *)(SA1100_ZEROS_BANK_BASE + SA1100_DCACHE_SIZE);
    ...
    while (ptr < ptr_stop)
    {
        temp = *(volatile unsigned long *)ptr;
        ptr += SA1100_DCACHE_LINESIZE;
    }

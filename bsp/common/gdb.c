...
_gdb_pkt_send(void)
{
	...
	*output_pkt_ptr = '\0';
	...
	asm volatile (".globl _bsp_interrupt_insn\n"
		  "_bsp_interrupt_insn:\n");
#else
	asm volatile (".globl bsp_interrupt_insn\n"
		      "bsp_interrupt_insn:\n");
#endif
	BREAKPOINT();
    }
...
send_stopped_packet (int sigval, void *regs)
{
	...
	for (i = 0; i < NUMREGS; i++) {
		...
		bsp_get_register(i, cur_regs, buf);
 ...
 * the breakpoint caused by reading a ^C while
 * sending a packet.
 */
extern void bsp_interrupt_insn(void);
...
_bsp_gdb_handler(int exc_nr, void *regs)
{
	...
	if (cur_pc == (unsigned long)bsp_breakinsn) {
	    ...
	    if (cur_pc == (unsigned long)bsp_interrupt_insn) {
		bsp_skip_instruction(regs);
	...
	switch (*ptr++) {
	  ...
	  case 'G':
		...
		ptr += bsp_regsize(i) * 2;
		bsp_set_register(i, cur_regs, buf);
	    }
	    _gdb_pkt_append("OK");
	    break;
	  ...
	  case 'P':
		...
		if (*(ptr++) == '=') {
		    ...
		    bsp_set_register(addr, cur_regs, buf);
		    _gdb_pkt_append("OK");
		    break;
		}
	    }
	  ...
	  case 'm': 
		...
		if (*(ptr++) == ',') {
			...
			for (i = 0; i < length; i++, ptr++) {
			    if (!bsp_memory_read(ptr, -1, 8, 1, &ch))
				break;
	  ...
	  case 'M':
		...
		if (*(ptr++) == ',') {
			...
			if (*(ptr++) == ':') {
				...
				i = bsp_memory_write((void*)addr, -1, 8, j, buf);
...
crc32 (unsigned char *ptr, int len)
{
    ...
    while (len--) {
	if (bsp_memory_read(ptr++, -1, 8, 1, &ch) <= 0) {
	    _gdb_pkt_append("E01");
	    return;
	}

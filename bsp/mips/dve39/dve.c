/*
 * dve.c -- Support for Densan DVE-R3900 board
 *
...
uart_irq_clear(struct schan *chan)
{
    ...
    junk = chan->control;
    __udelay(10);
    chan->control = RES_H_IUS; /* reset interrupt */
    __wbflush();
    junk = chan->control;
}
...
uart_irq_handler(int irq_nr, void *regs)
{
    ...
    /* !!FIXME!! Need 72001 manual to sort this
       out properly. It is obvilously different
       than 8350. */
     ...
     */
    UART_CH_A->control = 3;      /* get interrupt status */
    __wbflush();
	...
	if (uart_dbg_chan->control & Rx_CH_AV) {
	    ...
	    _bsp_enqueue(uart_dbg_chan == UART_CH_A ? &uart1_queue : &uart2_queue, ch);
	}
	uart_irq_clear(uart_dbg_chan);
	return 1;
#if 0
...
uart_control(void *chan_data, int func, ...)
{
	...
	if (uart_dbg_chan != chan) {
	    if (uart_dbg_chan == NULL) {
...
init_uart(int which, int bps)
{
    ...
    chan->control = 14;
    __wbflush();
    chan->control = DTRREQ | BRENABL | BRSRC;
    __wbflush();
}
...
void
__udelay(unsigned long n)
{
    unsigned long start = get_count();
    
    while ((get_count() - start) < n)
	;
}
...
{
#if defined(HAVE_NET)
    extern void __sed_InstallVector(void);
    __sed_InstallVector();    
#endif

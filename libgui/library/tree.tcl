...
itcl_class Tree {
	...
	pack $tree -side bottom -fill both -expand y
	
	if {$withframe} {
	    pack $lframe -side top -fill both -expand y
	}
	...
	$thisTail.size.btn$num config -text $txt
    }
	...
	for {set i 0} {$i <= $tabsize} {incr i} {
		...
		::bind $lframe.size.col$i <1>\
		    "$this start_motion $i %X"
		
		#motion process
		::bind $lframe.size.col$i <B1-Motion>\
		    "$this motion $i %X"
		...
		::bind $lframe.size.col$i <3>\
		    "$this toggle_splitlines"
	    }
	}
	...
	foreach W [array names resizing] {
		...
		set bef $f.y
		pack $f.y -side right -fill y -before $W
	    }

	    if {[$W xview] == "0 1"} {
		pack forget $f.x
	    } else {
		pack $f.x -side bottom -fill x -before $bef
	    }
	...
	set print_dialog [TopLevel [TopLevel::next_window_name]\
	    -leader $toplw]

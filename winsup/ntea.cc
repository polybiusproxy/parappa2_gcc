...
NTReadEARaw (HANDLE hFileSource, int *len)
{
  ...
  while (BackupRead (hFileSource, (LPBYTE) &StreamId, StreamSize,
	...
	if (StreamId.dwStreamId == BACKUP_EA_DATA)
	  {
	    unsigned char *buf;
	    buf = (unsigned char *) malloc (StreamId.Size.u.LowPart);

	    if (buf == NULL)
	      break;
	    if (!BackupRead (hFileSource, buf,	// buffer to read
			     StreamId.Size.u.LowPart, // num bytes to write
			     &dwBytesWritten,
			     FALSE,		// don't abort yet
			     FALSE,		// don't process security
			     &lpContext))
	    ...
	    *len = StreamId.Size.u.LowPart;
	    break;
	}
	/* Skip current stream */
	if (!BackupSeek (hFileSource,
			 StreamId.Size.u.LowPart,
			 StreamId.Size.u.HighPart,
			 &sl,
			 &sh,
			 &lpContext))
	  break;
...
NTWriteEA (const char *file, const char *attrname, char *buf, int len)
{
  ...
  StreamId.Size.u.LowPart = easize;

...
init_cplus_unsave ()
{
  ...
  lang_safe_for_unsave = cplus_safe_for_unsave;
  /* END CYGNUS LOCAL */
}
...
/* CYGNUS LOCAL sibcall/law */
/* The C++ version of safe_for_unsave.
   See gcc/tree.c:safe_for_unsave for comments. */

int
cplus_safe_for_unsave (expr)
     tree expr;
{
  if (expr == NULL_TREE)
    return 1;
    ...
    {
      if (! safe_for_unsave (TREE_OPERAND (expr,0))
	  || ! safe_for_unsave (TREE_OPERAND (expr,2)))
	return 0;

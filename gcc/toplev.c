...
/* Nonzero means use the edge based lcm for gcse from morgans Book.  */

int flag_edge_lcm = 1;

/* Nonzero means when using the edge based lcm for gcse from morgans Book, 
   also do the enhanced load motion, which trys to hoist loads by not
   killing loads because a store to the same location is seen.
   flag_edge_lcm is a pre-requisite for this option.  */

int flag_edge_lm = 1;

/* Nonzero means when using the edge based lcm for gcse from morgans Book, 
   also perform store motion, which will try to move stores closer to the
   exit block.  flag_edge_lcm is a pre-requisite for this option, and its
   not very effective without falg_edge_lm.  */
...
lang_independent_options f_options[] =
{
  ...
  {"edge-lcm", &flag_edge_lcm, 1,
   "Perform the global common subexpression elimination with edge based lcm" },
  {"edge-lm", &flag_edge_lm, 1,
   "Perform edge based load motion after global common subexpression elimination" },
  {"edge-sm", &flag_edge_sm, 1,
   "Perform edge based store motion after global subexpression elimination" },

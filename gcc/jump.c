     ...
     register rtx insn;
{
  ...
  /* CYGNUS LOCAL sibcall */
  /* If we simply delete a CALL_PLACEHOLDER, we will lose all BLOCK_BEG and
     BLOCK_END notes contained within the three alternative instruction
     sequences, causing us to blow up later in identify_blocks.  */
  if (GET_CODE (insn) == CALL_INSN
      && GET_CODE (PATTERN (insn)) == CALL_PLACEHOLDER)
    {
      ...
      /* The notes get emitted before INSN.  */
      prev = PREV_INSN (insn);
    }
  /* END CYGNUS LOCAL */

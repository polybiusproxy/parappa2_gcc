/* Parse C expressions for CCCP.
   Copyright (C) 1987, 1992, 1994, 1995, 1997, 1998, 1999 Free Software Foundation.
...
/* FIXME: This is a hack to work around a bug in 98r2 cygwin
   (__mb_cur_max isn't exported from the dll).  Lieing about
   MB_CUR_MAX is harmless here since it's only used to size
   one small buffer. */
     ...
     int olen;
{
  ...
  MAX_over_base = ((unsigned HOST_WIDEST_INT) -1) / ((unsigned HOST_WIDEST_INT) base);
  ...
  if ((HOST_WIDEST_INT) n < 0 && ! op.unsignedp)
    {
      if (base == 10)
	...
	if (! wide_flag)
	  {
		...
		op.value
		    = result & ((HOST_WIDEST_INT) ~0 >> (HOST_BITS_PER_WIDEST_INT - num_bits));
	    else
		op.value
		    = result | ~((HOST_WIDEST_INT) ~0 >> (HOST_BITS_PER_WIDEST_INT - num_bits));
	  }

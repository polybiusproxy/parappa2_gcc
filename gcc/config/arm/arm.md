...
(define_insn "*extendqihi_insn"
  ...
  /* After reload operand[1] can be converted into an illegal memory
     address because of frame pointer elimination.  This is not detected
     by reload, so we have to cope with it here.  (The split cannot be used
     because operands[1] no longer satisfies memory_operand()).  */
  else if (! memory_operand (operands[1], QImode))
    return arm_handle_bad_signed_byte_address (operands);
  /* END CYGNUS LOCAL */
  return \"ldr%?sb\\t%0, %1\";
"
[(set_attr "type" "load")
  ...
  "
{
  ...
  /* CYGNUS LOCAL nickc */
  else if (! ok_integer_or_other (operands[1]))
  /* END CYGNUS LOCAL */
    {

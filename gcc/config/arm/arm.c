...
arm_expand_prologue ()
{
  ...
  if (TARGET_APCS_STACK)
    {
      ...
      if (bound < 256)
      ...
      else
        {
          ...
          emit_insn (gen_addsi3 (gen_rtx_REG (SImode, 12),
                                 stack_pointer_rtx, bamount));
          emit_insn (gen_cond_call (gen_rtx_REG (SImode, 12),
                                    gen_rtx_REG (SImode, 10),
                                    gen_rtx_SYMBOL_REF (Pmode,
                                             "*__rt_stkovf_split_big"),
                                    gen_rtx (LTU, SImode, 24)));
  ...
  if (profile_flag || profile_block_flag || sp_overflow_check || TARGET_NO_SCHED_PRO)
    /* END CYGNUS LOCAL */
    emit_insn (gen_blockage ());
}
...
/* Return an instruction string that can handle a invalid
   operand[1] for the extendqisi_insn and extendqihi_insn
   patterns.  */
char *
arm_handle_bad_signed_byte_address (operands)
     rtx * operands;
{
  static char buffer [100];
  
#if 1  /* XXX no longer able to reproduce the bug fixed by enabling this code! */
  HOST_WIDE_INT val;

  /* Assumption: The invalid memory address is of the form:
     (plus (reg) (const_int))  */
  
  if (GET_CODE (XEXP (operands[1], 0)) != PLUS)
    abort ();
  
  if (GET_CODE (XEXP (XEXP (operands[1], 0), 1)) != CONST_INT)
    abort ();
  
  val = INTVAL (XEXP (XEXP (operands[1], 0), 1));
  
  /* Nothing clever here (for now).  Assume that the offset will fit
     into two bytes, add the high byte into the base register and place
     the result into the destination register, then load the destination
     register from itself plus the low byte of the offset.  */
  if (val > 0xFFFF)
    abort ();
  
  sprintf (buffer, "add%%?\t%%0, %%m1, #%d\n\tldr%%?sb\t%%0, [%%0, #%d]",
	   val & 0xFF00, val & 0x00FF);
#else
  sprintf (buffer, "ldr%%?sb\t%%0, %%1");
#endif
  return buffer;
}
/* END CYGNUS LOCAL */

...
;; If operand 3 is an eliminable reg, then register elimination causes
;; the pattern handled in "mul_acc_si_elim" below.  Unfortunately,
;; madd puts the result in the LO register, but the add needs inputs
;; from a general register.  We add a spurious clobber (of operand 7)
;; so that it will be present just in case register elimination gives
;; us the funny result.
...
(define_insn "*mul_acc_si"
  [(set (match_operand:SI 0 "register_operand" "=l,*d,*d")
   ...
   (clobber (match_scratch:SI 7 "=d,d,d"))]
  "(TARGET_MIPS3900
    || TARGET_MIPS5400)			/* CYGNUS LOCAL vr5400/raeburn */
   && !TARGET_MIPS16"
  "*
...
;; This can be created by register elimination if operand3 is an
;; eliminable register or has reg_equiv_constant set.

;; We have to use nonmemory_operand for operand 4 to ensure that the
;; validate_changes call inside eliminate_regs will always succeed.
;; If it doesn't succeed, this will remain a mul_acc_si pattern and
;; will be reloaded incorrectly.

(define_insn "*mul_acc_si_elim"
  [(set (match_operand:SI 0 "register_operand" "=l,*d,*d")
        (plus:SI (plus:SI (mult:SI (match_operand:SI 1 "register_operand" "d,d,d")
				   (match_operand:SI 2 "register_operand" "d,d,d"))
			  (match_operand:SI 3 "register_operand" "0,l,*d"))
		 (match_operand:SI 4 "nonmemory_operand" "")))
   (clobber (match_scratch:SI 5 "=h,h,h"))
   (clobber (match_scratch:SI 6 "=X,3,l"))
   (clobber (match_scratch:SI 7 "=a,a,a"))
   (clobber (match_scratch:SI 8 "=d,d,d"))]
  "reload_in_progress"
  "#"
...
(define_split
  [(set (match_operand:SI 0 "register_operand" "")
        (plus:SI (plus:SI (mult:SI (match_operand:SI 1 "register_operand" "")
				   (match_operand:SI 2 "register_operand" ""))
			  (match_operand:SI 3 "register_operand" ""))
		 (match_operand:SI 4 "nonmemory_operand" "")))
   (clobber (match_scratch:SI 5 ""))
  ...
  [(parallel [(set (match_dup 8)
		   (plus:SI (mult:SI (match_dup 1)
             ...
             (clobber (match_dup 7))
             (clobber (match_dup 8))])
   (set (match_dup 8) (plus:SI (match_dup 8) (match_dup 4)))

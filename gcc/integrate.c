     ...
     register tree fndecl;
{
  ...
  /* ??? Need to defer committing to the sibcall/tail recursion sequence
     after inlining.  A call site may be a sibling call in this function,
     but when this function is integrated it may not longer be a sibling
     call. 

     To make this work we need to have the code which saves the rtl handle
     CALL_PLACEHOLDERs (ie, save the rtl sequences and label attached to the
     CALL_PLACEHOLDER).  */
  for (insn = get_insns (); insn; insn = NEXT_INSN (insn))
    {
      if (GET_CODE (insn) == CALL_INSN && SIBLING_CALL_P (insn))
	return "function with sibling calls cannot be inlined at this time";
    }
  /* END CYGNUS LOCAL */
     ...
     tree fndecl;
{
  ...
  for (insn = NEXT_INSN (insn); insn; insn = NEXT_INSN (insn))
	...
	case NOTE:
	  ...
	  /* CYGNUS LOCAL sibcall/rth */
	  /* Do not keep these -- the saved bb pointer (which will
	     soon be dangling) confuses flow's attempts to preserve
	     bb structures during the compilation of a function.  */
     ...
     struct inline_remap *map;
{
  ...
  for (i = 0; i < GET_RTX_LENGTH (GET_CODE (copy)); i++)
	...
	case 'i':
	/* CYGNUS LOCAL sibcall/law */
	/* We need to handle 'n' for a "deleted" label which appears
	   in the DECL_RTL of a LABEL_DECL.  */
	case 'n':
	/* END CYGNUS LOCAL */

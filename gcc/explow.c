 ...
 restart:
  ...
  switch (code)
      ...
      else if (CONSTANT_P (XEXP (x, 1)))
	{
      ...
      /* Treating function invariants in the same way as constants
	 ensures that rtxes of the form (plus fp (const_int nn)) are
	 never broken apart. */
      else if (function_invariant_p (XEXP (x, 0)))
	{
	  x = gen_rtx_PLUS (mode,
			    plus_constant (XEXP (x, 0), c),
			    XEXP (x, 1));
      ...
      else if (function_invariant_p (XEXP (x, 1)))
	{
	  x = gen_rtx_PLUS (mode,
			    XEXP (x, 0),
			    plus_constant (XEXP (x, 1), c));

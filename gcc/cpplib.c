  ...
  if (nargs >= 0)
    {
      ...
      for (i = 0; i < nargs; i++)
	{
	  args[i].raw = args[i].expanded = args[i].stringified = 0;
	  args[i].raw_length = 0; 
	  args[i].expand_length = args[i].stringified_length = -1;
	  args[i].use_count = args[i].newlines = 0;
	}
      ...
      do
	{
	  ...
	  if (i < nargs || (nargs == 0 && i == 0))
		...
		rest_args = 1;
	      ...
	      args[i].newlines = (pfile->lineno - lineno);
	    }
	  else
    ...
    cpp_reader *pfile;
{
  ...
  for (;;)
	...
	{
	  cpp_error (pfile, "unterminated '#if' construct");
	  return;
	}

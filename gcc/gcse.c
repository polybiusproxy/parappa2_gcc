...
static void alloc_pre_mem	     PROTO ((int, int, int));
static void free_pre_mem	      PROTO ((void));
static void compute_pre_data	  PROTO ((int));
...
static struct ls_expr *ldst_entry 	PROTO ((rtx));
static void free_ldst_entry 		PROTO ((struct ls_expr *));
static void free_ldst_mems		PROTO ((void));
static void print_ldst_list 		PROTO ((FILE *));
static struct ls_expr *find_rtx_in_ldst	PROTO ((rtx));
static int enumerate_ldsts		PROTO ((void));
static inline struct ls_expr * first_ls_expr	PROTO ((void));
static inline struct ls_expr * next_ls_expr	PROTO ((struct ls_expr *));

static int simple_mem			PROTO ((rtx));
static void invalidate_any_buried_refs	PROTO ((rtx));
static void compute_ld_motion_mems	PROTO ((void)); 
static void trim_ld_motion_mems		PROTO ((void));
static void update_ld_motion_stores	PROTO ((struct expr *));


static void reg_set_info		PROTO ((rtx, rtx));
static int store_ops_ok			PROTO ((rtx, int));
static void find_moveable_store		PROTO ((rtx));
static int compute_store_table		PROTO ((void));
static int load_kills_store		PROTO ((rtx, rtx));
static int find_loads			PROTO ((rtx, rtx));
static int store_killed_in_insn		PROTO ((rtx, rtx));
static int store_killed_after		PROTO ((rtx, rtx, int));
static int store_killed_before		PROTO ((rtx, rtx, int));
static void build_store_vectors		PROTO ((void));
static void insert_insn_start_bb	PROTO ((rtx, int));
static int insert_store			PROTO ((struct ls_expr *, edge));
static void replace_store_insn		PROTO ((rtx, rtx, int));
static void delete_store		PROTO ((struct ls_expr *, int));
static void free_store_memory		PROTO ((void));
static void store_motion		PROTO ((void));
     ...
     FILE *file;
{
  ...
  /* Number of orignal basic blocks used if edge_lcm routines are called.  */
  int orig_bb_count;
  ...
  while (changed && pass < MAX_PASSES)
	...
	changed |= one_classic_gcse_pass (pass + 1);
	  ...
	  /* If we used edge_lcm, we have to recompute preds and succs.  */
	  if (flag_edge_lcm && changed)
	    {
	      int i;
  ...
  if (!optimize_size && flag_edge_lcm && flag_edge_sm)
    store_motion ();
	  ...
	  if (!optimize_size && equal_note && retval_note)
	    {
		  ...
		  /* If there is a function call within the libcall
		     block, add any call clobbered registers to the list.  */
		  if (contains_call)
		    {
		      int i;
			    ...
			    && !fixed_regs[i])
			  {
			    ...
			    extra_regs = gen_rtx_EXPR_LIST (VOIDmode,
							    reg,
							    extra_regs);
			  }
		    }
...
/* Nonzero for expressions which should be inserted in\\on a specific edge.  */
...
static void
alloc_pre_mem (n_blocks, n_exprs, using_edges)
     int n_blocks, n_exprs, using_edges;
{
     ...
     int pre;
{
  ...
  pat = process_insert_insn (expr, bb);
		  ...
		  for (occr = expr->antic_occr; occr != NULL; occr = occr->next)
						   ...
						   NULL)))
			{
			  ...
			  if ((eg->flags & EDGE_ABNORMAL) == EDGE_ABNORMAL)
			    ...
			    {
			      insn = process_insert_insn (index_map[j], 0);
			      insert_insn_on_edge (insn, eg);
			    }
			  if (gcse_file)
			    {
			      fprintf (gcse_file, "PRE/HOIST: edge (%d,%d),  copying expression %d\n",
				       bb, INDEX_EDGE_SUCC_BB (edge_list, e)->index, expr->bitmap_index);
			    }
      ...
      for (expr = expr_hash_table[i]; expr != NULL; expr = expr->next_same_hash)
	{
	  ...
	  for (occr = expr->antic_occr; occr != NULL; occr = occr->next)
	      ...
	      for (avail = expr->avail_occr; avail != NULL; avail = avail->next)
		{
		  ...
		  if (!flag_edge_lcm
		      && !TEST_BIT (temp_bitmap[bb], expr->bitmap_index))
		    continue;
		  ...
		  if (forced_dependant_expressions (expr))
		    {
			 ...
			 gcse_pre_insert.  */
			  ...
			  delete_libcalls = alloc_EXPR_LIST (VOIDmode, insn,
			        			     delete_libcalls);
					  ...
					  expr->reaching_reg, 0))
			{
			  ...
			  rtx x = delete_libcalls;
			  delete_libcalls = XEXP (delete_libcalls, 1);
			  free_EXPR_LIST_node (x);
			  delete_insn (insn);
			}
		    }
		}
...
pre_gcse ()
{
  ...
  if (flag_edge_lcm)
    { 
      int did_insert = pre_edge_insert (edge_list, index_map);
     ...
     int pass;
{
  ...
  alloc_expr_hash_table (max_cuid);
  /* Only use the ld_motion list with edge based lcm.  */
  if (flag_edge_lcm)
    { 
  ...
  if (n_exprs > 0)
    {
      alloc_pre_mem (n_basic_blocks, n_exprs, flag_edge_lcm);
      compute_pre_data (flag_edge_lcm);
      if (flag_edge_lcm)
        {
	  changed |= pre_gcse ();
...
/*  Here we provide the things required to do store motion towards
    ...
    load towards the exit, and we end up with no laods or stores of 'i'
    in the loop.
...
trim_ld_motion_mems ()
{
  ...
  while (ptr != NULL)
    {
      int del = ptr->invalid;   /* Delete if entry has been made invalid.  */
     ...
     struct expr *expr;
{
  ...
  if (mem_ptr = find_rtx_in_ldst (expr->expr))
      ...
      for ( ; list != NULL_RTX; list = XEXP (list, 1))
	{
	  ...
	  /* Can I do this?? , or do I need to create a new insn for
	     the store? I'd like to leave the same insn since this store is
	     in lists, and I dont want to issue this more than I have to.  
	     All stores to this mem are going to change to this, so even if
	     its shared rtl it should matter??  */
	  SET_SRC (pat) = reg;
     ...
     int bb;
{
 ...
 repeat:
  ...
  switch (code)
    {
    case REG:
        /* If a reg has changed after us in this block, the operand has been
	   killed.  */
	return TEST_BIT (reg_set_in_block[bb], REGNO (x));
...
build_store_vectors () 
{
  ...
  /* Any block with no exits calls some non-returning function, so
     ...
     ot at all.  If we knew it was abort, we wouldn't have to store,
     but we don't knkow that for sure. */
     ...
     int bb;
{
    ...
    if (GET_RTX_CLASS (GET_CODE (before)) != 'i')
	before = NEXT_INSN (before);
    insn = emit_insn_before (insn, before);
    if (before == BLOCK_HEAD (bb))
      BLOCK_HEAD (bb) = insn;
    set_block_num (insn, bb);
    if (gcse_file)
      {
